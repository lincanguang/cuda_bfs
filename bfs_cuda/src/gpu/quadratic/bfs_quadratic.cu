...
__global__
void compute_next_layer_distance(int n, int *adjacencyList, int *edgesOffset, 
		int *edgesSize, int *distance, int level, bool *done) {
...
void bfsGPUQuadratic(int start, Graph &G, vector<int> &distance, vector<bool> &visited) {
	...
	printf("Elapsed time for quadratic GPU implementation (without copying graph) : %li micro seconds.\n", duration);
