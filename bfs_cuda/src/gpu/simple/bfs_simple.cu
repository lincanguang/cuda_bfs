...
void myComputeNextQueue(int *adjacencyList, int *edgesOffset, int *edgesSize, int *distance,
	int *queue, int front, int cursize, int *qsize, int level, int n) {
	const int tid = blockIdx.x * blockDim.x + threadIdx.x;
	if (tid < cursize) {
		volatile int *cur_q = queue + (front % n) ;
		volatile int *next_q = queue + ((front+ cursize) % n);
		int current = cur_q[tid];
		for (int i = edgesOffset[current]; i < edgesOffset[current] + edgesSize[current]; ++i) {
			int v = adjacencyList[i];
			if (distance[v] == INT_MAX) {
				distance[v] = level + 1;
				int pos = atomicAdd(qsize, 1);
				next_q[pos] = v;
			}
		}
		__syncthreads();
...
void mybfsGPU(int start, Graph &G, vector<int> &distance, vector<bool> &visited) {
	...
	*h_nextQueueSize = NEXT_QUEUE_SIZE;
	...
	while (currentQueueSize > 0) {
		...
		computeNextQueue << <n_blocks, N_THREADS_PER_BLOCK >> > (d_adjacencyList, d_edgesOffset, d_edgesSize, d_distance,
			currentQueueSize, d_currentQueue, h_nextQueueSize, d_nextQueue, level);
		cudaDeviceSynchronize();
		++level;
	...
	auto endTime = std::chrono::steady_clock::now();
	auto duration = chrono::duration_cast<chrono::microseconds>(endTime - startTime).count();
	printf("Elapsed time for my GPU implementation (without copying graph) : %li micro second.\n", duration);
